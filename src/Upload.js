...
const Upload = () => {
  ...
  const [userChallengeOptions, setUserChallengeOptions] = useState([]);
  ...
  useEffect(() => {
    const fetchSounds = async () => {
      const { data, error } = await supabase
        .from("sounds")
        .select("id, title, audio_url, profiles(user_name, avatar_url)");
  ...
  useEffect(() => {
    const fetchJoinedChallenges = async () => {
      ...
      if (error) {
        ...
        const challengeList = data.map((entry) => ({
          id: entry.challenges.id,
          title: entry.challenges.title,
        }));
        setUserChallengeOptions(challengeList);
  ...
  const handleUploadSound = async () => {
    ...
    const fileName = `${user.id}_${Date.now()}.${soundFile.name
      .split(".")
      .pop()}`;
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from("sounds").getPublicUrl(filePath);

    await supabase
      .from("sounds")
      .insert([{ title, audio_url: publicUrl, user_id: user.id }]);
    setUploading(false);
    navigate("/home");
  };
            ...
            <div className="max-w-xl mx-auto space-y-3">
              {thumbnail && (
                <img
                  src={thumbnail}
                  alt="Thumbnail"
                  className="rounded-lg w-48 h-auto mx-auto shadow-md"
                />
              )}
              <textarea
                value={caption}
              ...
              {selectedSound && (
                <audio src={selectedSound} controls className="w-full" />
              )}
              {uploading && (
                <div className="mb-4">
              ...
              <button
                ...
                disabled={uploading}
                className="w-full py-2 bg-yellow-500 text-black font-bold rounded"
              >
